...
#include <thread>
#include <tof_base/algebraic/transformations.h>
...
private:
    ...
    Eigen::Matrix4d camera_T_target_;
...
void PoseAnalysis::setParams()
{
    ...
    pnh_->param<bool>(pnh_->getNamespace() + "/pose_transform/apply_transform", apply_transform_, false);
...
Eigen::Vector3d PoseAnalysis::getPoseError(const geometry_msgs::PoseStamped& _est_pose)
{
    ...
    if (apply_transform_) {
        /*
            gt_T_est = gt_T_tgt * tgt_T_cam * cam_T_est

            gt = ground truth frame
            est = estimate frame
            tgt = transformed camera frame
            cam = camera frame
        */
        rel_pose = experiment_gt_.inverse() * camera_T_target_.inverse() * est_pose.matrix();
    } else {
        /*
            gt_T_est = gt_T_cam * cam_T_est
...
void PoseAnalysis::applyZeroMeanError()
{
    ...
    Eigen::MatrixXd xyz_rpy = Eigen::MatrixXd::Zero(6, 1);
    xyz_rpy.topLeftCorner(3, 1) = pose_error_map.rowwise().mean();
    Eigen::Matrix4d error_T_zero = algebraic::createTransform<double>(xyz_rpy).matrix();
    /*
        [x' y' yaw' 1]^T = (  zero_T_error * [x y yaw 1]^T  ).topRows(3)

        x' = the x error after zero-mean transformation.
        y' = the y error after zero-mean transformation.
        yaw' = the yaw error after zero-mean transformation.
        error_T_zero = the zero-mean transformation contributed from x y yaw distributions.
    */
    pose_error_map = (error_T_zero.inverse() * pose_error_map.colwise().homogeneous()).topRows(3);
}
